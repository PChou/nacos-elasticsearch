...
/**
 * Naming proxy.
 * NOTES: The following code is copy from nacos-client/1.3.2, and omit the refresh task.
 *        Since elasticsearch plugin running under strict java security context managed by elasticsearch,
 *        Each thread `must` wrap privilege context before doing some security sensitive operation such as
 *        socket, reflect, file access... The original  refresh task is not correctly do this, so we decide
 *        to remove the refresh task and put it out of proxy itself
 * @author nkorange
 */
